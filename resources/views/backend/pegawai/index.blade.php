                  ...
                  <tbody>
                    @forelse ($pegawais as $key => $value)
                        <tr>
                          ...
                          <td>
                            <form action="{{ route('pegawai.destroy', $value->id) }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin menghapus data ini?')">
                              @csrf
                              @method('DELETE')
                              <a href="{{ route ('pegawai.edit', $value->id) }}" class="btn btn-warning btn-sm">Edit</a>
